...
def test_browser_import_setup_edit_template_and_mobile(browser_app):
    ...
    page.wait_for_function("document.querySelectorAll('.symbol-upload img').length===4")
    ...
    page.wait_for_function("!!document.querySelector('#back-upload img')")
    page.fill('#deck-artist','Deck Artist');page.click('#save-setup');page.wait_for_function("document.querySelector('#setup-state')?.textContent.startsWith('Saved settings')")
    page.click('[data-tab=cards]');page.locator('[data-card]').first.click();page.fill('#card-qty','3');page.click('#save-card')
    page.wait_for_function("document.querySelector('.quantity-pill')?.textContent==='3×'")
...
def test_real_cardconjurer_roundtrip(tmp_path):
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            page.wait_for_function("document.querySelector('.badge.ready') || document.querySelector('.toast.error')",timeout=240000)
