...
async function request(t,kind,offset){
  ...
  try{
    ...
    const r=await fetch(t.origin+'/api/transfer/'+t.id+'/'+kind,{headers,signal:controller.signal,cache:'no-store',credentials:'omit'});
  ...
  (async()=>{
    try{
      if(message.type==='PF_WORKSPACE_OPEN_ORDER'){
        ...
        if(meta.zipBytes>1024**3)throw new Error('This order exceeds the helper’s 1 GB transfer limit. Split the order or download the ZIP for manual upload.');
      ...
      if(message.type==='PF_ORDER_METADATA'){respond({ok:true,count:t.count,zipBytes:t.zipBytes,filename:t.filename||'Proxy_Foundry_Order.zip',version:VERSION});return;}
      if(message.type==='PF_ORDER_CHUNK'){
        if(message.offset>=t.zipBytes)throw new Error('Requested chunk is outside the saved order.');
        const chunk=await request(t,'zip',message.offset);if(chunk.total!==t.zipBytes)throw new Error('The order changed during transfer. Stop and reopen it.');respond({ok:true,...chunk});return;
      }
