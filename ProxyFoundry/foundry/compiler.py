...
AUTO_TEMPLATE_VERSIONS.update({'saga':7,'saga-creature':5,'class':3,'transform-front':6,'transform-back':6,'station':2,'meld':3,'land':2,'legendary-land':2})
...
def apply_colorless_land_frame_treatment(data,sem):
    """Neutralize colored template defaults when a land produces no W/U/B/R/G."""
    if 'Land' not in set(sem.get('types',[])) or sem.get('land_colors'):return False
    # Cover any ordinary M15 pieces the preserved compiler knows how to recolor.
    try:native.recolor_m15(data,'L')
    except native.BuildError:pass
    changed=False
    for frame in data.get('frames',[]):
        if not isinstance(frame,dict):continue
        src=str(frame.get('src') or '')
        new=src
        # Generic-showcase land texture is the neutral land treatment used by
        # the existing colorless land family.
        new=re.sub(
            r'/img/frames/m15/genericShowcase/m15GenericShowcaseFrame[WUBRGMALC]\\.png
        ...
        if choice in {x['id'] for x in BUILTINS}:
            ...
            if group in {'transform-front','transform-back'}:
                ...
                try:
                    ...
                    if group in {'land','legendary-land'}:
                        apply_colorless_land_frame_treatment(data,sem)
...
,
            '/img/frames/m15/genericShowcase/m15GenericShowcaseFrameL.png',
            new,
        )
        # Legendary-land compact accents use color+land filenames (ul/rl/etc.).
        if re.fullmatch(r'/img/frames/m15/boxTopper/short/(?:[wubrgma]?l|[wubrgma])\\.png',new):
            new='/img/frames/m15/boxTopper/short/l.png'
        # Legendary crowns must likewise become the neutral Land crown.
        new=re.sub(
            r'/img/frames/m15/crowns/m15Crown[WUBRGMALC](Floating(?:Alt)?)?\\.png
    """Use muted color accents while retaining each artifact's structural shell."""
    types=set(sem.get('types',[]))
    if 'Artifact' not in types:return False
    colors=[]
...
class Compiler:
    ...
    def compile_face(self,sf,face,index,options,settings,art_id,*,art_origin='custom artwork'):
        ...
        return {'name':sem['name'],'data':data,'renderKey':key,'render':self.store.render_get(key),'group':group,'recipe':recipe,'crop':warning,'flags':flags,'artId':art_id,'symbolId':symbol_id,'artist':str(artist),'credit':credit,'generationVersion':PIPELINE_VERSION,'templateKey':template_key,'templateVersion':template_version,'templateCacheVersion':template_cache_version}
,
            lambda m:'/img/frames/m15/crowns/m15CrownL'+(m.group(1) or '')+'.png',
            new,
        )
        if new!=src:
            frame['src']=new;changed=True
            if 'name' in frame:
                masks={m.get('name') for m in frame.get('masks',[]) if isinstance(m,dict)}
                piece=next(iter(masks & {'Pinline','Title','Type','Rules','Border'}),None)
                frame['name']=('Neutral Land '+piece) if piece else frame['name'].replace('Blue','Land').replace('White','Land').replace('Black','Land').replace('Red','Land').replace('Green','Land').replace('Multicolored','Land')
    return changed
