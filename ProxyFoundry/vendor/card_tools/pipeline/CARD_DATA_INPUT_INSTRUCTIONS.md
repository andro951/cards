...
The wrapper excludes `Sideboard`, `Outside the Game`, and `Maybeboard` by
default. `Sideboard` and `Maybeboard` remain excluded. `Outside the Game` can
be included explicitly with `--include-outside-the-game`. Scryfall currently
exports that UI section under the JSON key `outside`; both `outside` and
`outside_the_game` are recognized aliases.
...
#### `loyalty`
If Scryfall supplies loyalty, preserve it as a string. The compiler currently supports only the calibrated four-ability tall-textbox Planeswalker treatment; normal Planeswalker layouts remain intentionally unsupported until separately tested.
...
## Saga geometry and review behavior

Saga support is now enabled in the automatic compiler. Preserve the following approved measurements and behaviors:

- Standard horizontal title/footer geometry follows a normal card.
- Set symbol X: **1737 px**.
- Saga set symbol Y: **2400 px**.
- Saga art window: **X 1005, Y 316, width 854, height 2041** on the 2010 x 2814 canvas.
- Preferred new Saga art size: **854 x 2041 px**.
- Saga art fitting uses Card Conjurer's normal cover/center-crop equation based on the actual source image dimensions, rather than hardcoded X/Y/scale values.
- The manual portrait-art fit established during testing was **X 752, Y 316, scale 132.9%** for that image; this is an output of the equation, not a universal hardcoded preset.

The currently checked Saga layout families are the ones represented by the six approved reference cards:
...
If a future Saga uses a different chapter-group layout, do **not** reject the whole batch. Compile the Saga anyway and add it to the sidecar JSON report named like `*_FLAGGED_UNCHECKED_SAGAS.json`.
...
Exception: for Saga cards only, unmatched/new chapter-group layouts should compile and be listed in the `*_FLAGGED_UNCHECKED_SAGAS.json` sidecar instead of rejecting the batch.
...
## Scryfall deck-section filtering

The deck wrapper excludes these Scryfall sections by default:

- `Sideboard`
- `Outside the Game` (Scryfall export key: `outside`)
- `Maybeboard`

Use `--include-outside-the-game` when that section should be part of the proxy
batch. Sideboard and Maybeboard remain excluded. Section matching is
case/spacing/punctuation insensitive. `--section` can narrow included sections
further, but Outside the Game still requires the explicit include flag.
