...
def build(destination):
    ...
    manifest={'application':'Proxy Foundry','version':'1.2.0','files':{str(p.relative_to(ROOT)).replace('\\','/'):hashlib.sha256(p.read_bytes()).hexdigest() for p in files}}
...
if __name__=='__main__':
    import sys
    destination=Path(sys.argv[1] if len(sys.argv)>1 else ROOT.parent/'ProxyFoundry_1_2.zip')
