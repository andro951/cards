...
export async function showDeck(id,tab='cards'){
  ...
  let v=views.get(id);if(!v){v={query:'',filter:'all',flipped:new Set()};views.set(id,v);}
  ...
  $('#main').innerHTML=`<a class="back-to-library" href="#decks">← All decks</a><div class="page-head"><div><span class="eyebrow">DECK STUDIO</span><h1>${esc(d.name)}</h1><div class="actions" style="margin-top:12px">${badge(d.status)}<span class="count-label">${summary.cards} cards · ${summary.faces} faces · ${summary.rendered} rendered</span></div><div class="deck-progress"><span class="complete"><b>✓</b>Import</span><span class="${dirty?'current':'complete'}"><b>${dirty?'2':'✓'}</b>Art & style</span><span class="${d.status==='ready'?'complete':!dirty?'current':''}"><b>${d.status==='ready'?'✓':'3'}</b>Generate images</span><span><b>4</b>Review & print</span></div></div><div class="actions"><button class="button" id="deck-menu">More ▾</button><button class="button" id="add-cards">＋ Add cards</button><button class="button primary" id="generate-deck">Generate images</button></div></div>
    <div class="tabs"><button data-tab="cards" class="${tab==='cards'?'active':''}">Cards <span>${summary.cards}</span></button><button data-tab="setup" class="${tab==='setup'?'active':''}">Art & setup</button><button data-tab="review" class="${tab==='review'?'active':''}">Review <span>${summary.warnings+summary.errors||''}</span></button></div><div id="deck-body"></div>`;
  ...
  function cardsGrid(){
    ...
    $('#deck-body').innerHTML=`${dirty?'<div class="notice info">You have saved changes to prepare. Generate images will rebuild only the affected faces and reuse unchanged PNGs.</div>':''}<div class="toolbar"><div class="filter-pills"><button data-card-filter="all" class="${v.filter==='all'?'active':''}">All cards</button><button data-card-filter="unrendered" class="${v.filter==='unrendered'?'active':''}">Not rendered</button><button data-card-filter="attention" class="${v.filter==='attention'?'active':''}">Needs review</button></div><label class="search"><input id="card-search" type="search" aria-label="Find a card" placeholder="Find a card…" value="${esc(v.query)}"></label><button class="button small" id="quick-order">Review order ↗</button></div>${cards.length?`<div class="card-grid">${cards.map(c=>{
      const f=c.faces[0],flipped=v.flipped.has(c.id),url=flipped?backPreview(c,d):preview(c,f),done=flipped?!!(c.backOverride||c.faces[1]?.compiled?.render||c.faces.length===1&&d.settings.backAsset):!!f.compiled?.render;
      const warning=c.faces.some(x=>x.error||x.compiled?.crop?.warning||x.compiled?.flags?.length);
      return `<article class="card-item"><button class="card-image" data-card="${c.id}" aria-label="Edit ${esc(c.name)}"><span class="quantity-pill">${c.quantity}×</span>${warning?'<span class="warn-pill">!</span>':''}${url?`<img src="${esc(url)}" class="${done?'':'art-only'}" loading="lazy" alt="${esc(c.name)}${flipped?' back':''}">`:'<span class="card-empty-symbol">▱</span>'}${!done?`<span class="image-label">${flipped?'No back selected':f.error?'Needs attention':'Art preview · not rendered'}</span>`:dirty?'<span class="image-label">Previous render · changes pending</span>':''}</button><h3 title="${esc(c.name)}">${esc(c.name)}</h3><div class="card-item-footer"><small>${esc((c.scryfall.set||'').toUpperCase())} ${esc(c.scryfall.collector_number||'')} · ${esc(flipped?'Back':f.compiled?.render?'Rendered':'Not rendered')}</small><button class="flip-button" data-flip="${c.id}" aria-label="Show ${flipped?'front':'back'} of ${esc(c.name)}">${flipped?'Front':'Back'} ↻</button></div></article>`;
    }).join('')}</div>`:empty(d.cards.length?'No cards match':'This deck is waiting for cards',d.cards.length?'Try a different search or review filter.':'Add a card list or a Scryfall export to get started.',`<button class="button primary" id="empty-add">＋ Add cards</button>`)}<div class="subtitle-line">Click a card to edit its artist, art, printing, quantity or template. Real double-faced cards keep their reverse as a paired face.</div>`;
    $$('[data-card]').forEach(b=>b.onclick=()=>attempt(()=>inspect(d,d.cards.find(c=>c.id===b.dataset.card),0)));
    $$('[data-flip]').forEach(b=>b.onclick=()=>{v.flipped.has(b.dataset.flip)?v.flipped.delete(b.dataset.flip):v.flipped.add(b.dataset.flip);const scroll=$('.card-grid')?.scrollTop;cardsGrid();if($('.card-grid'))$('.card-grid').scrollTop=scroll||0;});
...
function deckMenu(d){
  ...
  $('#use-as-defaults').onclick=()=>attempt(async()=>{const defaults={symbols:d.settings.symbols,backAsset:d.settings.backAsset,artist:d.settings.artist,templateRules:d.settings.templateRules};await api('/api/settings',{defaults});closeModal();toast('Symbols, back, artist and templates saved as defaults for new decks.');});
  $('#trash-deck').onclick=()=>attempt(async()=>{closeModal();if(!await confirmAction('Move this deck to Trash?',d.name+' and its saved setup can be restored later.','Move to Trash',true))return;await api('/api/decks/'+d.id+'/delete',{revision:d.revision});state.selected.delete(d.id);nav('decks');});
}
async function inspect(deck,card,index=0){
  ...
  let f=c.faces[index]||c.faces[0],fit={...(f.fit||{})},artOverride=f.artOverride,backOverride=c.backOverride;let changedArt=false;
  ...
  <div class="field"><span>Back override <small>optional, applies to this card only</small></span><div class="actions"><button class="button small" id="face-back">Upload back</button><button class="button quiet small" id="clear-face-back">${c.faces.length===2?'Use actual reverse':'Use deck back'}</button></div><small id="face-back-state">${backOverride?'Individual back selected':c.faces.length===2?'Paired with '+esc(c.faces[1].name):'Using deck default back'}</small></div>
  ...
  $('#face-art').onclick=()=>attempt(async()=>{const file=await pickFile();if(!file)return;artOverride=(await uploadImage(file)).id;changedArt=true;fit={};$('#face-art-state').textContent='Custom art: '+file.name;if($('#inspector-image'))$('#inspector-image').src=asset(artOverride);});
  $('#clear-face-art').onclick=()=>{artOverride=null;changedArt=true;fit={};$('#face-art-state').textContent='Using deck source after regeneration';};
  $('#face-back').onclick=()=>attempt(async()=>{const file=await pickFile();if(!file)return;backOverride=(await uploadImage(file)).id;$('#face-back-state').textContent='Custom back: '+file.name;});
  $('#clear-face-back').onclick=()=>{backOverride=null;$('#face-back-state').textContent=c.faces.length===2?'Actual reverse will be used':'Deck default back will be used';};
  ...
  $('#save-card').onclick=async()=>{
    try{
      const artist=$('#blank-credit').checked?'':$('#face-artist').value||null;
      ...
      const patch={revision:d.revision,quantity:$('#card-qty').value,backOverride:backOverride||null,faceId:f.id};
      const changes={artistOverride:artist,artOverride:artOverride||null,templateOverride:$('#face-template').value||null,fit,semanticOverrides:semantic};
