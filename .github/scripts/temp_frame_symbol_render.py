...
OUT = ROOT / 'test-results' / 'frame-symbol-render'
WORKSPACE = ROOT / 'test-results' / 'frame-symbol-workspace'
...
def png_image(size=(1200, 900), seed=0):
    """Simple deterministic fixture art; CardConjurer still does all card rendering."""
...
try:
    ...
    print('PREPARED FRAME RECIPES')
    for c in deck['cards']:
        comp = c['faces'][0]['compiled']
        data = comp['data']
        print(c['name'], 'group=', comp['group'], 'recipe=', comp['recipe'], 'version=', data.get('version'),
              'symbol=', data.get('setSymbolX'), data.get('setSymbolY'), data.get('setSymbolZoom'))
    ...
    # Diagnostic manifest is left beside the artifact for CI logs, but the user ZIP contains exactly five PNGs.
    (OUT / 'manifest.json').write_text(json.dumps(manifest, indent=2), encoding='utf-8')
    zip_path = OUT / 'black-square-frame-tests.zip'
