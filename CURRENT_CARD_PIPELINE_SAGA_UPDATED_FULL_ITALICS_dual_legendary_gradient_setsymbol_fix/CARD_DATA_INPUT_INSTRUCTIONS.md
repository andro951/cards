...
The wrapper always excludes Scryfall's non-deck sections: `Sideboard`,
`Outside the Game`, and `Maybeboard`. Scryfall currently exports the UI label
`Outside the Game` under the JSON key `outside`; both `outside` and
`outside_the_game` are recognized as excluded aliases. These exclusions remain
in force even when repeated `--section` arguments are used to select specific
playable sections.
...
The deck wrapper always excludes these Scryfall sections from generated card data and Card Conjurer batches:

- `Sideboard`
- `Outside the Game` (Scryfall export key: `outside`)
- `Maybeboard`

Section matching is case/spacing/punctuation insensitive. Commander/partner/companion/mainboard and other non-excluded playable sections remain eligible. `--section` can narrow the included sections further, but cannot opt these three excluded sections back in.
